  ...
  return (
    <div className="flex flex-col gap-4 h-full min-h-0">
      {competitorMatch && productData && (
        <div className="p-4 rounded-xl bg-gradient-to-r from-finastra-purple/20 to-finastra-fuchsia/10 border border-finastra-purple/30 flex items-start gap-4 shadow-[0_0_15px_rgba(105,78,214,0.15)] animate-in fade-in slide-in-from-bottom-2 duration-500 shrink-0">
      ...
      <div className="w-full flex-1 min-h-0 overflow-y-auto overflow-x-hidden rounded-xl border border-white/5 shadow-2xl bg-black/40 relative">
      <table className="w-full text-left border-collapse">
        <thead className="sticky top-0 z-10">
          <tr className="border-b border-white/10 text-xs font-bold text-slate-400 uppercase tracking-widest bg-slate-900/90 backdrop-blur-md">
            <th className="py-4 px-4 font-semibold">Institution Name</th>
            <th className="py-3 px-3 font-semibold text-right">Total Assets</th>

            {isProductMode ? (
              <>
                <th className="py-3 px-4 font-semibold text-center">{selectedProduct} Fit</th>
                <th className="py-3 px-4 font-semibold text-left">Justification</th>
              </>
            ) : (
              <>
                <th className="py-3 px-4 font-semibold text-center">Top Product Fit</th>
                <th className="py-3 px-4 font-semibold text-center">Top Product Match</th>
              </>
            )}
          </tr>
        </thead>
          ...
          {banks.map((bank, idx) => {
            ...
            return (
              ...
              >
                ...
                <td className="py-3 px-3  text-right text-sm text-slate-300 font-medium">
