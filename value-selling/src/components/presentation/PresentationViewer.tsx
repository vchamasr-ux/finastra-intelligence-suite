...
export const PresentationViewer: React.FC = () => {
  ...
  const demographicRevenueUplift = Math.max(0, (selectedBank.NETLNLS || selectedBank.LNLSNET || ASSET_B * 1000 * 0.6) / 1000 * demoRevenueUpliftRate * 1000);
  ...
  return (
    <div className="flex-1 bg-slate-100 flex flex-col h-full w-full font-body overflow-hidden print:overflow-visible print:bg-white print:block">
      {/* Navigation Bar */}
      <div className="h-16 bg-f-charcoal text-white flex items-center justify-between px-6 shrink-0 z-50 shadow-md print:hidden">
        <button className="flex items-center gap-2 hover:text-f-fuchsia transition text-sm font-semibold tracking-wide font-heading">
          <X size={18} /> CLOSE PRESENTATION
        </button>
        <div className="text-xs font-bold tracking-widest text-[#8a99a8] flex items-center gap-3 font-heading uppercase flex-1 justify-center px-4">
          <span className="truncate max-w-[200px] xl:max-w-[300px]" title={selectedBank.NAME}>{selectedBank.NAME}</span>
          <span className="text-[#3e4c5b]">•</span>
          <span className="truncate max-w-[150px] xl:max-w-[200px]" title={selectedProduct["Product/solution"]}>{selectedProduct["Product/solution"]}</span>
          <span className="text-[#3e4c5b] opacity-0 md:opacity-100">•</span>
          <span className="whitespace-nowrap">SLIDE {currentSlide + 1} OF {TOTAL_SLIDES}</span>
        </div>
        <div className="flex items-center gap-4 font-heading">
          <div className="flex bg-[#2a3645] rounded overflow-hidden shadow-inner w-32 h-8 relative mr-6">
          ...
          <button onClick={handlePrint} className="flex items-center gap-2 px-3 py-1.5 border border-[#3e4c5b] rounded hover:bg-[#2a3645] transition text-sm">
            <Download size={16} /> Save as PDF
          </button>
          <button onClick={toggleFullScreen} className="flex items-center gap-2 px-3 py-1.5 border border-[#3e4c5b] rounded hover:bg-[#2a3645] transition text-sm">
            <Maximize size={16} /> Full Screen
          </button>
          <div className="flex gap-2 ml-4">
            <button onClick={() => setCurrentSlide(p => Math.max(0, p - 1))} disabled={currentSlide === 0}
              className="px-4 py-1.5 bg-[#2a3645] rounded hover:bg-[#3e4c5b] disabled:opacity-50 text-sm font-semibold transition">Prev</button>
            <button onClick={() => setCurrentSlide(p => Math.min(TOTAL_SLIDES - 1, p + 1))} disabled={currentSlide === TOTAL_SLIDES - 1}
              className="px-4 py-1.5 bg-gradient-to-r from-f-violet to-f-fuchsia text-white rounded hover:opacity-90 disabled:opacity-50 text-sm font-bold transition shadow-lg">Next</button>
          </div>
        </div>
      </div>

      {/* Slide Canvas — fully fluid and responsive, filling the remaining height */}
      <div className="flex-1 w-full bg-slate-100 flex flex-col overflow-hidden min-h-0 relative print:overflow-visible print:block">
        <div 
          className="w-full h-full bg-white relative shadow-sm flex flex-col @container overflow-hidden print:shadow-none print:overflow-visible print:block"
        >
