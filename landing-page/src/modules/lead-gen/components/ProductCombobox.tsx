  ...
  return (
    <div className={`relative ${className}`} ref={containerRef}>
      ...
      <button
        ...
        className="w-full h-[38px] flex items-center justify-between pl-9 pr-4 text-sm font-medium rounded-lg border transition-all bg-slate-900/80 border-slate-800/80 text-slate-300 hover:border-indigo-500/50 focus:outline-none focus:ring-1 focus:ring-indigo-500/50 shadow-inner"
      >
        <Package className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-500" />
        <span className="truncate">{value === 'all' || !value ? 'All Products' : value}</span>
        <ChevronDown className={`w-4 h-4 text-slate-500 transition-transform ${isOpen ? 'rotate-180' : ''}`} />
      </button>

      {/* Popover */}
      {isOpen && (
        <div className="absolute z-50 w-full mt-2 bg-slate-900 border border-slate-700/80 rounded-xl shadow-2xl flex flex-col max-h-[400px] shadow-indigo-500/10 transform origin-top animate-in fade-in zoom-in-95 duration-200">
          <div className="p-2 border-b border-slate-800 relative shrink-0">
            <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-500" />
            <input 
              type="text" 
              className="w-full bg-slate-1000/50 border border-slate-800 text-slate-200 text-sm rounded-lg pl-9 pr-3 py-2 outline-none focus:border-indigo-500/50 focus:ring-1 focus:ring-indigo-500/50 placeholder:text-slate-600 transition-all font-medium"
          ...
          <div className="overflow-y-auto flex-1 p-1 custom-scrollbar">
            {/* 'All Products' option */}
            {(!search || "all products".includes(search.toLowerCase())) && (
              <button
                type="button"
                className={`w-full text-left flex items-center px-3 py-2 text-sm rounded-md mb-2 transition-all ${value === 'all' || !value ? 'bg-indigo-500/20 text-indigo-400 font-semibold shadow-sm' : 'text-slate-300 hover:bg-slate-800 hover:text-slate-100'}`}
                onClick={() => { onChange('all'); setIsOpen(false); setSearch(''); }}
              >
                <div className="flex-1 flex items-center gap-2">
                  <div className={`w-2 h-2 rounded-full ${value === 'all' || !value ? 'bg-indigo-400 shadow-[0_0_8px_theme(colors.indigo.400)]' : 'bg-transparent'}`} />
                  All Products
                </div>
                  ...
                  {products.map(p => {
                    ...
                    return (
                      <button
                        key={p}
                        type="button"
                        className={`w-full text-left flex items-center px-3 py-2 text-sm rounded-md transition-all group ${isSelected ? 'bg-indigo-500/10 text-indigo-400 font-semibold' : 'text-slate-300 hover:bg-slate-800 hover:text-slate-100'}`}
