export async function fetchAppRating(bankName: string, fdicCert?: string): Promise<number | null> {
  ...
  if (text.trim().startsWith('<')) {
    throw new Error('[iTunes Pipeline Fatal] Received HTML instead of JSON. Ensure itunes_cache.json is bundled in the public directory and dev server is restarted.');
  }
